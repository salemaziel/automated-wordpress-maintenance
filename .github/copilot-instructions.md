...
- **Backups exclude transient cache.** `BACKUP_EXCLUDE_DIRS` (e.g. `wp-content/cache`, `wp-content/upgrade`) is omitted from the backup tar, the rollback forensic snapshot, and the disk-space estimate. The estimate measures the excluded dirs separately and subtracts, rather than using `du --exclude`, so the estimate equals what tar writes by construction — `du` and `tar` match their exclude patterns against different strings, and a silent mismatch would under-reserve disk. `--no-backup-excludes` restores full-archive behaviour. Rollback recreates the excluded dirs after extract, because `advanced-cache.php` is restored and its drop-in expects them.
